...
// Tajawal font - Fetch from jsDelivr CDN
const tajawalFontUrl = 'https://cdn.jsdelivr.net/npm/@fontsource/tajawal@5.0.8/files/tajawal-arabic-400-normal.ttf';

// Generate PDF with jsPDF using Tajawal font
window.generatePDF = async function() {
  try {
    // Clear any cached font to avoid corruption
    sessionStorage.removeItem('tajawalFontBase64');

    if (typeof window.jspdf === 'undefined' || typeof window.jspdf.jsPDF === 'undefined') {
      throw new Error('فشل تحميل مكتبة jsPDF. تأكد من اتصالك بالإنترنت.');
    }
    ...
    // Try to load Tajawal font
    try {
      const fontResponse = await fetch(tajawalFontUrl, { cache: 'force-cache' });
      if (!fontResponse.ok) throw new Error('فشل تحميل خط Tajawal');
      ...
      // Cache font
      sessionStorage.setItem('tajawalFontBase64', fontBase64);
    } catch (error) {
      console.warn('Failed to load Tajawal font:', error.message);
      // Fallback to Helvetica
      doc.setFont('Helvetica');
      showNotification('خط Tajawal غير متاح، يتم استخدام خط افتراضي.', 'warning');
    }
    ...
    doc.autoTable({
      ...
      startY: 80,
      styles: {
        font: fontLoaded ? 'Tajawal' : 'Helvetica',
        fontSize: 12,
        textColor: '#FFD700',
        fillColor: '#1A1A1A',
        halign: 'right'

...
// Arabic font (Amiri) - Fetch from jsDelivr CDN
const amiriFontUrl = 'https://cdn.jsdelivr.net/npm/@fontsource/amiri@5.0.8/files/amiri-arabic-400-normal.ttf';

// Generate PDF with jsPDF or html2pdf as fallback, using FileSaver.js for download
window.generatePDF = async function() {
  try {
    // Clear any cached font to avoid corruption
    sessionStorage.removeItem('amiriFontBase64');
    ...
    if (typeof window.jspdf !== 'undefined' && typeof window.jspdf.jsPDF !== 'undefined') {
      ...
      // Try to load Amiri font
      try {
        const fontResponse = await fetch(amiriFontUrl);
        if (!fontResponse.ok) throw new Error('فشل تحميل خط Amiri');
        ...
        // Cache font
        sessionStorage.setItem('amiriFontBase64', fontBase64);
      } catch (error) {
        console.warn('Failed to load Amiri font:', error.message);
        // Fallback to Helvetica in jsPDF
        doc.setFont('Helvetica');
        showNotification('الخط العربي غير متاح، يتم استخدام خط افتراضي.', 'warning');
      }

      // Gradient Background (Approximated)
      doc.setFillColor(18, 18, 18); // #121212
      doc.rect(0, 0, 595, 842, 'F'); // A4 size
      ...
      if (!pdfBlob || !(pdfBlob instanceof Blob) || pdfBlob.size === 0 || pdfBlob.type !== 'application/pdf') {
      ...
      try {
        ...
        // Fallback to manual link as last resort
        ...
        }, 60000); // 60 seconds for manual download
      }
    } else {
      // Fallback to html2pdf.js with English translation
      if (typeof html2pdf !== 'undefined') {
        showNotification('فشل تحميل jsPDF، يتم إنشاء PDF بالإنجليزية.', 'warning');

        // Create a temporary container for the PDF content
        const tempContainer = document.createElement('div');
        tempContainer.style.padding = '20px';
        tempContainer.style.backgroundColor = '#121212';
        tempContainer.style.color = '#FFD700';
        ...
        title.textContent = 'NileVibe Weekly Schedule';
        ...
        const headerRow = document.createElement('tr');
        ['Day', 'Morning', 'Afternoon', 'Evening'].forEach(text => {
          const th = document.createElement('th');
          th.textContent = text;
          th.style.padding = '10px';
          th.style.backgroundColor = '#121212';
          th.style.color = '#FFD700';
          th.style.border = '1px solid #FFD700';
        ...
        footer.textContent = 'Powered by NileVibe - Discover Cairo in Luxury';
        ...
        // Save PDF and get blob
        const pdfOutput = await html2pdf().set(opt).from(tempContainer).output('blob');
        ...
        try {
          ...
          showNotification('تم تحميل البرنامج كـ PDF بالإنجليزية! تحقق من مجلد التحميلات.', 'success');
        } catch (saveError) {
          console.error('FileSaver Error:', saveError);
          // Fallback to manual link as last resort
        ...
        throw new Error('فشل تحميل كل من jsPDF وhtml2pdf. تأكد من اتصالك بالإنترنت.');
      }
    }
  } catch (error) {
    showNotification(
